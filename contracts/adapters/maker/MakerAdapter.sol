...
abstract contract MKRAdapter {
    ...
    function mkrRmul(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = mkrMul(x, y) / ONE;
    }

    function mkrMul(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require(y == 0 || (z = x * y) / y == x);
    }

    function mkrAdd(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require((z = x + y) >= x);
    }
}

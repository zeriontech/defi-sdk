...
Once a protocol is added to **ProtocolAdapterRegistry** contract, it will immediately appear in all the interfaces that use DeFi SDK \(including Zerion\). In order to add a protocol to DeFi SDK, one has to implement the following contracts.

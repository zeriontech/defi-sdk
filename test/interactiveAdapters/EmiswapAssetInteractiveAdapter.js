...
const EMISWAP_ADAPTER = web3.eth.abi.encodeParameter(
  'bytes32',
  web3.utils.toHex('Emiswap LP'),
).slice(0, -2);
...
const EMISWAP_ASSET_ADAPTER = `${EMISWAP_ADAPTER}${ASSET_ADAPTER}`;
const EMISWAP_EXCHANGE_ADAPTER = `${EMISWAP_ADAPTER}${EXCHANGE_ADAPTER}`;
...
const EmiswapAdapter = artifacts.require('./EmiswapAssetInteractiveAdapter');
...
contract('EmiswapAssetInteractiveAdapter', () => {
  ...
  beforeEach(async () => {
    ...
    await EmiswapAdapter.new({ from: accounts[0] })
      .then((result) => {
        protocolAdapterAddress = result.address;
      });
    await EmiswapExchangeAdapter.new({ from: accounts[0] })
      .then((result) => {
        emiswapAdapterAddress = result.address;
      });
    ...
    await protocolAdapterRegistry.methods.addProtocolAdapters(
      [
        EMISWAP_ASSET_ADAPTER,
        EMISWAP_EXCHANGE_ADAPTER,
        WETH_ASSET_ADAPTER,
      ],
      [
        protocolAdapterAddress,
        emiswapAdapterAddress,
        wethAdapterAddress,
      ],
      [
        [wethDaiAddress],

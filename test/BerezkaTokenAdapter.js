...
contract("BerezkaTokenAdapter", _ => {
  it("...should return corrent amount of tokens when called via adapter registry", async () => {
    ...
    // Deploy governance
    //
    const governance = await BerezkaTokenAdapterGovernance.new(adapterRegistry.address, [], [])
    ...
    //
    await governance.addTokens('Uniswap V2 pool token', [uniswapPool])
    await governance.addTokens('ERC20', [dai])

    // Deploy berezka adapter with governance
    //
    const berezkaTokenAdapter = await BerezkaTokenAdapter.new(governance.address)
    await adapterRegistry.addTokenAdapters(
      ['Berezka DAO'],
